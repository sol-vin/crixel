class Crixel::QuadLeaf
  ...
  def subdivide
    ...
    w = width/2
    h = height/2
    nw = QuadLeaf.new(x, y, w, h)
    ne = QuadLeaf.new(x + w, y, w, h)
    sw = QuadLeaf.new(x, y + h, w, h)
    se = QuadLeaf.new(x + w, y + h, w, h)
  ...
  class QuadTree < QuadLeaf
    alias Item = NamedTuple(item: Basic, leafs: Set(QuadLeaf))
    getter children = [] of Item
    getter total_checks = 0
    getter max_children = 5
    getter max_depth = 6

    def initialize(@max_depth = 6)
    end

    def includes?(child : Basic)
      @children.any?(&[:item].equals(child))
    end

    def insert(child : Basic) : Set(QuadLeaf)
      ...
      if self.includes?(child)
        item = @children.find(&[:item].equals?(child))
        @children.delete item
      end

      quads_inside = Set(QuadLeaf).new

      if divided?
        bounds = child.as(IBody)
        quads_inside.concat @nw.not_nil!.insert(child) if bounds.intersects?(@nw.not_nil!)
        quads_inside.concat @ne.not_nil!.insert(child) if bounds.intersects?(@ne.not_nil!)
        quads_inside.concat @sw.not_nil!.insert(child) if bounds.intersects?(@sw.not_nil!)
        quads_inside.concat @se.not_nil!.insert(child) if bounds.intersects?(@se.not_nil!)
      else
        if @children.size+1 > max_children && !(@current_depth + 1 > max_depth)
          bounds = child.as(IBody)
          quads_inside.concat @nw.not_nil!.insert(child) if bounds.intersects?(@nw.not_nil!)
          quads_inside.concat @ne.not_nil!.insert(child) if bounds.intersects?(@ne.not_nil!)
          quads_inside.concat @sw.not_nil!.insert(child) if bounds.intersects?(@sw.not_nil!)
      ...
      @children << {item: child, leafs: quads_inside}
